...
source
资源文件夹是存放用户资源的地方。除 _posts 文件夹之外，开头命名为_ (下划线)的文件 / 文件夹和隐藏的文件将会被忽略。Markdown 和 HTML 文件会被解析并放到 public 文件夹，而其他文件会被拷贝过去。
...
官网主题(模板)： <https://hexo.io/themes/，在上面选择自己喜欢的模板，进入它对应的git页面，获得clone>的链接。
...
如果你已经配置了自己的域名，在_config.yml里修改url为自己的域名，例如我的<http://liyuankun.top/>。
...
``` bash
remote: GitHub found 1 vulnerability on CatherineLiyuankun/Hexo-theme-zilan default branch (1 moderate). To find out more, visit:
remote:      https://github.com/CatherineLiyuankun/Hexo-theme-zilan/security/dependabot/package.json/hexo/open
```

进去一刻发现是提醒因为安全漏洞应该升级hexo版本
> CVE-2021-25987
> moderate severity
> Vulnerable versions: >= 0.0.1, <= 5.4.0
> Patched version: No fix
> Hexo versions 0.0.1 to 5.4.0 are vulnerable against stored XSS. The post “body” and “tags” don’t sanitize 
> malicious javascript during web page generation. Local unprivileged attacker can inject arbitrary code.

好，那就升级。`package.json` 文件里的hexo版本，改成最新的6.00：`"hexo": "^6.0.0"`
然后 `rm -rf node_modules && npm install --force`, `npm audit fix`

``` bash
hexo s
(node:30592) ExperimentalWarning: The fs.promises API is experimental
...
先看报错： `FATAL TypeError: Object.fromEntries is not a function`， [网上一查，发现是nodejs版本问题](https://github.com/algolia/gatsby-plugin-algolia/issues/106)，用大于等于12的nodejs版本就没问题了。
`nvm install 12`
`nvm use 12`
再执行`hexo s`， 果然好嘞。
...
进入https://hexo.io/docs/configuration，搜索"external_link" ， 现在配置变为`external_link.enable`
所以_config.yml里面的`external_link: true`改成：
...
再执行`hexo s`， 果然没有Warning了。

### `hexo s`之后在页面请求出现报错

```bash
Unhandled rejection TypeError: /Users/liyuankun/GitRepos/LYKGit/Blog/Hexo-theme-zilan/themes/zilan/layout/page.ejs:41
    39|             </div>
    40|
 >> 41|           <%- partial(_partial/sidebar) %>
    42|
    43|         <%} %>
    44|     </div>
```
...
```bash
/Users/liyuankun/GitRepos/LYKGit/Blog/Hexo-theme-zilan/themes/zilan/layout/_partial/sidebar.ejs:9
 >> 9|     <% theme.widgets.forEach(function(widget){ %>
    10|         <%- partial(_widget/ + widget) %>
    11|         <hr>
    12|     <% }) %>
...
hexo-fs                                1.0.2   1.0.2   3.1.0  hexo-site
hexo-generator-archive                 0.1.5   0.1.5   1.0.0  hexo-site
hexo-generator-category                0.1.3   0.1.3   1.0.0  hexo-site
hexo-generator-feed                    1.2.2   1.2.2   3.0.0  hexo-site
hexo-generator-index                   0.2.1   0.2.1   2.0.0  hexo-site
hexo-generator-searchdb                1.3.3   1.4.0   1.4.0  hexo-site
hexo-generator-seo-friendly-sitemap   0.0.25  0.0.25   0.2.1  hexo-site
hexo-generator-sitemap                 2.1.0   2.2.0   2.2.0  hexo-site
hexo-generator-tag                     0.2.0   0.2.0   1.0.0  hexo-site
hexo-math                              3.0.4   3.0.4   4.0.0  hexo-site
hexo-renderer-ejs                      1.0.0   1.0.0   2.0.0  hexo-site
hexo-renderer-markdown-it              3.4.1   3.4.1   6.0.0  hexo-site
hexo-renderer-stylus                   0.3.3   0.3.3   2.0.1  hexo-site
hexo-server                            0.2.2   0.2.2   3.0.0  hexo-site
```
...
- [Hexo升级与问题排除(TypeError: line.matchAll is not a function)](https://blog.h1msk.cc/2021/09/25/Hexo%E5%8D%87%E7%BA%A7%E4%B8%8E%E9%97%AE%E9%A2%98%E6%8E%92%E9%99%A4-TypeError-line-matchAll-is-not-a-function/)
- [升级一下 Hexo](https://zguishen.com/posts/6821e493.html)
